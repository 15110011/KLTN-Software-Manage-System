...
def send_email_api(user, to_address, from_address, subject, message, step_detail_id):
    ...
    step_detail.thread.append({'thread_id':res['thread_id'], 'type': 'Send Email'})
    step_detail.save()
    #except:
    #    pass
                    ...
                    if step.id == instance.step.id:
                        ...
                        if "Send Email" in steps[index+1].actions:
                            ...
                            queue.enqueue(send_email_api, self.context.get(
                                'request').user, cur_contact.mail , "theaqvteam@gmail.com",steps[index+1].mail_template.subject, handle_mail_template.manipulate_template(steps[index+1].mail_template.template, contact=cur_contact), step_detail.id+1)
                            break
        return step_detail
