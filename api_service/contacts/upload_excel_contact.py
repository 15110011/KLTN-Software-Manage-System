...
def upload_excel_contacts(request):
    ...
    if target_group:
        groups = models.ContactGroup.objects.get(id=target_group)
        df = pd.read_excel(request.data['file'], header=1)
        contacts = []
        for d in df.values:
            try:
                #cur_contact = models.Contact(first_name=d[0], last_name=d[1], mail=d[2], phone=d[3], sex=d[4].upper(), address=d[5], country='USA', state=reverse_state_hashes[d[6]], city=d[7], zipcode=d[8], org=d[9], user=request.user)
                cur_contact = {"first_name": d[0], "last_name": d[1], "mail": d[2], "phone": str(0)+str(d[3]), "sex": d[4].upper(
                ), "address": d[5], "country": 'USA', "state": reverse_state_hashes[d[6]], "city": d[7], "zipcode": d[8], "org": d[9], "user": request.user, "groups": [groups]}
                ...
                if serializer.errors.get('non_field_errors', None):
                    ...
                    return Response({"validation_erros": serialier.errors, "first_name": d[0], "last_name": d[1]}, status=status.HTTP_400_BAD_REQUEST)
        #default_group = models.ContactGroup.objects.get(id=target_group)
        #err_contacts = []
        # for c in contacts:
        #    try:
        #        default_group.contacts.add(c)
        #    except:
        #        err_contacts.append(
