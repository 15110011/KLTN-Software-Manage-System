...
function Report(props) {
  ...
  const [dataSearch, setDataSearch] = React.useState({list: [], success_rate: 0, no_campaign: 0});
  ...
  const handleFilter = e => {
    apiPost(
      ...
      {
        data: {
          state: data.states.length ? data.states : null,
  ...
  return (
    <div className={classes.root}>
      <Grid container>
            ...
            {user.profile.is_manager && (
              <Grid item xs={3}>
                ...
                >
                  ...
                  <MenuItem value="sale">Sale Rep</MenuItem>
                </TextField>
              </Grid>
            )}
            <Grid item xs={2}>
        ...
        <Grid item xs={3}>
          <h5>Total campaigns: {dataSearch.no_campaign}</h5>
        </Grid>
        <Grid item xs={3}>
          <h5>Total orders: {dataSearch.list.length}</h5>
        </Grid>
        <Grid item xs={3}>
          <h5>Success rate: {Math.round(dataSearch.success_rate * 100)}%</h5>
        </Grid>
        <Grid item xs={3}>
          <h5>Income: ${new Intl.NumberFormat('de-DE').format(dataSearch.list.reduce((acc, cur)=>{
            acc+= cur.income
            return acc
          },0))}</h5>
        </Grid>
        <Grid item xs={12} className="pt-3">
          {typeReport == "product" ? (
            <MaterialTable
              ...
              columns={[
                ...
                },
                {
                  ...
                  customSort: (a, b) => {
                    ...
                    return a.product.toString().toLowerCase() <
                ...
                },
                {
                  ...
                  customSort: (a, b) => {
                    ...
                    return a.campaign.toString().toLowerCase() <
                      b.campaign.toString().toLowerCase()
                      ? -1
                      : 1;
                  }
                },
              ...
              data={
                dataSearch &&
                dataSearch.list.map((s, i) => ({
                  order: s.id ? s.id : "",
                  product:
                    s.campaign.product != null
                      ? s.campaign.product.name
                      : "asad",
