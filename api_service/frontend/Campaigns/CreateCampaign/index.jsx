...
function CreateCampaign(props) {
  ...
  const [saleRep, setSaleRep] = useFetchData(
    GET_SALE_REPS_URL,
    props.history,
    {}
  );
  ...
  const handleApplyConditionTable = e => {
    ...
    apiPost(
      ...
      {
        contacts: contactId,
        conditions: createCampaign.marketing_plan.condition.must
      },
      false,
      true
    ).then(res => {
  ...
  const handleCreateCampaign = e => {
    if (startDate > endDate) {
      ...
      if (
        isCreateMarketingPlanDialog == false &&
        ...
        if (
          data.marketing_plan.actions &&
          data.marketing_plan.actions.findIndex(a => a == "Send Email") != -1
        )
  ...
  const fetchProductSuggestion = input => {
    if (input != "") {
      return apiGet(PRODUCTS_URL + "?product_suggest=" + input, true).then(
        res => {
  ...
  const handleChangeProductSelect = (value, action) => {
    if (action.action == "input-change") {
      ...
      apiGet(PRODUCTS_URL + "?name=" + value.value, true).then(res => {
        ...
        apiGet(PACKAGES_URL + "?searchProduct=" + realResult.id, true).then(
          res => {
            setCreateCampaign({
              ...createCampaign,
  ...
  const fetchMarketingPlanSuggestion = input => {
    return apiGet(
      MARKETING_PLANS_URL + "?marketing_plan_suggest=" + input,
      true
    ).then(res => {
      return res.data.suggestion.map(s => ({ label: s, value: s }));
    });
  ...
  const addMarketingPlanToEdit = createdMarketingPlan => {
    setCreateCampaign({
      ...createCampaign,
      marketing_plan: createdMarketingPlan
    });
  };
  ...
  const addFollowUpPlanToEdit = createdFollowUpPlan => {
    setCreateCampaign({
      ...createCampaign,
      follow_up_plan: createdFollowUpPlan
    });
  };
  ...
  const fetchPackageSuggestion = input => {
    if (input != "") {
      return apiGet(PACKAGES_URL + "?package_suggest=" + input, true).then(
        res => {
          return res.data.suggestion.map(s => ({ label: s, value: s }));
        }
      );
  ...
  const handleChangePackageSelect = (value, action) => {
    if (action.action == "input-change") {
      ...
      apiGet(PACKAGES_URL + "?name=" + action.option.value, true).then(res => {
        ...
        apiGet(PRODUCTS_URL + "?name=" + realResult.product.name, true).then(
          res => {
            const realResultProduct = res.data.data[0];
            apiGet(
              PACKAGES_URL + "?searchProduct=" + realResultProduct.id,
              true
            ).then(res => {
              setCreateCampaign({
                ...createCampaign,
                packagesOptions: res.data.data,
                product: realResultProduct,
  ...
  const apiPostCampaign = data => {
    apiPost(CAMPAIGNS_URL, data, false, true).then(res => {
      if (res.data.code == "token_not_valid") {
        apiPost(REFRESH_TOKEN_URL, {
          refresh: localStorage.getItem("refresh")
        }).then(res => {
  ...
  const fetchFollowUpPlanSuggestion = input => {
    return apiGet(
      FOLLOW_UP_PLANS_URL + "?followup_plan_suggest=" + input,
      true
    ).then(res => {
      return res.data.suggestion.map(s => ({ label: s, value: s }));
    });
  ...
  const {
    ...
    handleCloseCreateCampaignDialog,
    setCreateCampaignDialog
  } = props;
        ...
        >
          ...
          >
            ...
            <div style={{ position: "absolute", top: "12px", right: "12px" }}>
              <IconButton
                aria-label="Close"
                onClick={handleCloseCreateCampaignDialog}
              >
