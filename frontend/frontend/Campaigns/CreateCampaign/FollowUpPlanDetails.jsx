...
function FollowUpPlanDetails(props) {
  ...
  if (createCampaign.follow_up_plan.steps) {
    createCampaign.follow_up_plan.steps.forEach((s, index) => {
      checkBoxOrRadio[index] = s.conditions.some(
        c => c.type == "check_box" || c.type == "radio"
      );
    });
  }
  ...
  return (
    <Grid container spacing={24}>
      ...
      <Grid item xs={6}>
        <AsyncSelect
          handleChange={(values, element) =>
            handleChangeFollowUpPlanSelect(values, element)
          }
          ...
          data={
            Object.keys(createCampaign.follow_up_plan).length === 0
              ? ""
              : {
                  label: `${createCampaign.follow_up_plan.name}`,
                  value: createCampaign.follow_up_plan.id,
                  ...createCampaign.follow_up_plan
                }
          }
          single
          placeholder=""
      ...
      {createCampaign.follow_up_plan.steps && (
        <>
          ...
          <Grid item xs={6}>
            ...
            >
              {createCampaign.follow_up_plan.steps &&
                createCampaign.follow_up_plan.steps.map((s, index) => {
      ...
      <Grid item xs={2} />
      {createCampaign.follow_up_plan.steps &&
        viewingOrder !== createCampaign.follow_up_plan.steps.length - 1 && (
          <Grid item xs={8}>
            <Paper className="p-4">
              <Grid container spacing={8} className="text-left">
                <Grid item xs={6}>
                  <FormControl>
                    ...
                    <FormGroup>
                      {actions.actions &&
                        actions.actions.reduce((acc, g) => {
                          acc.push(
                            <FormControlLabel
                              control={
                                <Checkbox
                                  checked={createCampaign.follow_up_plan.steps[
                                    viewingOrder
                                  ].actions.find(a => a == g)}
                                  disabled
                                />
                ...
                {createCampaign.follow_up_plan.steps[viewingOrder]
                  .email_template_ && (
                  <Grid item xs={6}>
                    <FormControl>
                      <FormLabel disabled component="legend">
